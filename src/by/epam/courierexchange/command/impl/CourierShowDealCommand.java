...
public class CourierShowDealCommand implements Command {
    ...
    public String execute(SessionRequestContent content) throws CommandException {
        ...
        try {
            findClientOffer.addAll(new ClientOfferRespondedServiceImpl().findByCourierAndStatus(user, OfferStatusType.IN_PROCESS));
            findResponded.addAll(new CourierOfferRespondedServiceImpl().findByCourier(user, OfferStatusType.IN_PROCESS));
            findWorkingOfferClient.addAll(new ClientOfferRespondedServiceImpl().findByCourierAndStatus(user, OfferStatusType.WORKING));
            findWorkingOfferCourier.addAll(new CourierOfferRespondedServiceImpl().findByCourier(user, OfferStatusType.WORKING));
            findCompletedOfferClient.addAll(new ClientOfferRespondedServiceImpl().findByCourierAndStatus(user, OfferStatusType.COMPLETED));
            findCompletedOfferCourier.addAll(new CourierOfferRespondedServiceImpl().findByCourier(user, OfferStatusType.COMPLETED));
