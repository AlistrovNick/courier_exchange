...
import by.epam.courierexchange.dao.CourierOfferDao;
...
public enum CourierOfferDaoImp implements CourierOfferDao {
    INSTANCE;

    private static Logger logger = LogManager.getLogger();
    private final static String SQL_SELECT_BY_OFFER_ID = "SELECT courier_offer.status, users.user_id, users.first_name, users.last_name FROM courier_offer INNER JOIN users ON courier_offer.user_id=users.user_id WHERE courier_offer.id=?;";
    private final static String SQL_SELECT_BY_USER = "SELECT id, status FROM courier_offer WHERE user_id=?;";
    ...
    @Override
    public List<CourierOffer> find(int offerId) throws DaoException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQL_SELECT_BY_OFFER_ID)) {
            ...
            while (resultSet.next()) {
                ...
                courierOffer.setId(offerId);
                courierOffer.setStatus(OfferStatusType.valueOf(resultSet.getString(STATUS).toUpperCase()));
                User user = new User();
    ...
    @Override
    public List<CourierOffer> find(User user) throws DaoException {
        List<CourierOffer> courierOfferList = new ArrayList<>();
        connection = ConnectionPool.INSTANCE.getConnection();
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQL_SELECT_BY_USER)){
    ...
    @Override
    public List<CourierOffer> find(OfferStatusType status) throws DaoException {
        List<CourierOffer> courierOfferList = new ArrayList<>();
        connection = ConnectionPool.INSTANCE.getConnection();
    ...
    @Override
    public List<CourierOffer> find(User user, OfferStatusType status) throws DaoException {
        List<CourierOffer> findList = new ArrayList<>();
        connection = ConnectionPool.INSTANCE.getConnection();
